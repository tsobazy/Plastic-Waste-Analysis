...
Note: We noticed that all_countries_data have duplicated rows. So, we handle this by making each row unique.
...
```{r}
# Merge plastics and population API datatset
...
Note: We noticed that all_countries_data2 have duplicated rows. So, we handle this by making each row unique.
...
```{r}
#Merge plastics and demographic API dataset
